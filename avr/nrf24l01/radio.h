...
/**
 * Configure the auto-ack feature in sending mode
 */
void Radio_Autoack(RADIO_USE_ACK ack);
 ...
 * \param use_aa Enable or diable Auto Ack
 * \param payload_width How many bytes should be received
 */
void Radio_Configure_Rx(RADIO_PIPE pipe, uint8_t* address, ON_OFF enable, RADIO_USE_ACK use_aa, uint8_t payload_width);
 ...
 * \param wait If this is RADIO_WAIT_FOR_TX, then Radio_Transmit will not return until the transmission is completed
 * 		successfully or fails.  If this is RADIO_RETURN_ON_TX then Radio_Transmit will return as soon as the
 ...
 * \return If wait was RADIO_RETURN_ON_TX, then Radio_Transmit always returns RADIO_TX_SUCCESS.  If wait was
