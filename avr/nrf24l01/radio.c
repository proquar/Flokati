...
void Radio_Autoack(RADIO_USE_ACK ack) {
	uint8_t value;
	if (ack==RADIO_ACK) value = 0x15;	//5 retransmits every 500us
	else value=0;
	
	//set_register(SETUP_AW, &value, 1); // probably wrong
...
/////// Code for AT90USB
// 	DDRE &= ~_BV(PORTE7);		// set pin as input
// 	EICRB |= _BV(ISC71);		// falling edge generates interrupt
// 	EICRB &= ~_BV(ISC70);		// (see above)
// 	EIMSK |= _BV(INT7);			// 

/////// Code for Atmega88, INT0
	DDRD &= ~_BV(PORTD2);
	EICRA |= _BV(ISC01);
	EICRA &= ~_BV(ISC00);
	EIMSK |= _BV(INT0);
...
void Radio_Configure_Rx(RADIO_PIPE pipe, uint8_t* address, uint8_t enable, RADIO_USE_ACK use_aa, uint8_t payload_width)
{
	uint8_t value;
	//uint8_t use_aa = 1;
